...
* **Imputación de Valores Nulos:** Uso de algoritmos como `KNNImputer` basado en similitud geoespacial[cite: 5].
* **Encoding de Variables Categóricas:** Manejo de alta cardinalidad utilizando `TargetEncoder` (con suavizado) para variables complejas y `LabelEncoder` para el resto[cite: 5].
* **Modelado y Ensamblado:** Entrenamiento de un `VotingClassifier` mediante "soft voting", combinando la robustez de un modelo `RandomForestClassifier` y un `XGBClassifier`[cite: 5]. Se aplicó también la técnica de Random Over Sampling para corregir el desbalanceo severo de clases[cite: 5].